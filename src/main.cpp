...
int main(int, char **) {
    ...
    while (!done) {
    	...
    	static int last_width = 1600, last_height = 900;

    	// In your main loop after SDL_PollEvent:
    	int current_width, current_height;
    	SDL_GetWindowSize(window, &current_width, &current_height);

    	if (abs(current_width - last_width) > 20 || abs(current_height - last_height) > 20) {
    		float scale = sqrtf((float)(current_width * current_height) / (1600.0f * 900.0f));
    		scale = ImClamp(scale, 1.0f, 2.0f);
