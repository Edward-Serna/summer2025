...
// No need for #undef main in SDL3
...
int main(int, char **) {
    ...
    SDL_Window *window = SDL_CreateWindow("Summer 2025", 1600, 900,
    ...
    // Fixed font loading with proper path and error checking
    ...
    const char* font_path = "thirdparty/imgui/misc/fonts/Roboto-Medium.ttf";
    ImFont* roboto_font = io.Fonts->AddFontFromFileTTF(font_path, scaled_font_size);
    if (roboto_font == nullptr) {
        printf("Warning: Failed to load Roboto font from '%s', using default font\n", font_path);
    }
